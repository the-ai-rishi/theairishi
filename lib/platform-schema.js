...
function collectPlatformConfigErrors(raw, vis) {
  ...
  if (!brand || typeof brand !== "object") {
    ...
    if (brand.tagline != null && typeof brand.tagline !== "string") {
      errors.push("brand.tagline, when present, must be a string. Leave it empty if the brand has no tagline.");
    }
  }
  ...
  } else if (!isNonEmptyString(raw.defaults.authorName)) {
    errors.push("defaults.authorName is required (used as the Person name in structured data)");
  }
