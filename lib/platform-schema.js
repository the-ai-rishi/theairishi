...
const { collectSocialErrors } = require("./social");
...
function collectPlatformConfigErrors(raw, vis) {
  ...
  if (!nav || !Array.isArray(nav.main) || !Array.isArray(nav.footer)) {
    ...
    for (const listName of ["main", "footer"]) {
      ...
      for (const item of nav[listName]) {
        ...
        if (item.placement != null && item.placement !== "primary" && item.placement !== "explore") {
          errors.push(
            "navigation." + listName + " item '" + item.id + "' placement must be primary or explore"
          );
        }
        if (vis && vis.normalizeStatus) {
          const status = vis.normalizeStatus(item.status || "active");
          if (status === "planned" || status === "coming-soon") {
            errors.push(
              "navigation." +
                listName +
                " item '" +
                ...
                "' is enabled but " +
                status +
                ". Set enabled false until the destination is real, or set status active."
            );
          }
        }
      }
  ...
  const socialErrors = collectSocialErrors(raw.social);
  for (const err of socialErrors) errors.push(err);
