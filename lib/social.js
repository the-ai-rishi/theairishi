 ...
 *
 * Telegram may use one documented temporary URL while the real
 * community does not exist yet. That URL can appear as a destination
 * but is never emitted in JSON-LD sameAs. Do not invent a t.me group.
 */
...
/** Documented Telegram stand-in. Not a community. Replace with https://t.me/... later. */
...
function destinationUrl(channel) {
  ...
  if (isTelegramTemporaryUrl(url)) return url;
  ...
  for (const channel of social) {
      ...
      if (enabled && status === "active") {
        if (!combined) {
          ...
          if (channel.id !== "telegram") {
            errors.push("The documented Telegram placeholder can only be used on social id telegram.");
          }
    ...
    if (channel.id === "telegram") {
      if (enabled && status === "active") {
        if (!combined) {
          errors.push(
            "social 'telegram' is enabled but url is empty. Use the documented placeholder " +
              TELEGRAM_TEMPORARY_URL +
              " or paste the real https://t.me/... URL."
          );
        } else if (isTelegramTemporaryUrl(combined)) {
          if (channel.includeInSameAs === true) {
            errors.push(
              "social 'telegram' is using the temporary placeholder. Set includeInSameAs to false until the real https://t.me/... URL is pasted."
            );
          }
        } else if (!isRealTelegramCommunityUrl(combined)) {
          errors.push(
            "social 'telegram' needs the documented placeholder " +
              TELEGRAM_TEMPORARY_URL +
              " or a real https://t.me/... community URL. Do not invent a group."
          );
        }
      }
    }
