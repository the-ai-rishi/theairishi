...
function isVisibleOnSurface(entity, surface, options) {
  ...
  if (surface === "search" || surface === "sitemap") {
    if (surface === "search" && entity.includeInSearch === false) return false;
    if (surface === "sitemap" && entity.includeInSitemap === false) return false;
    if (requireContent && !hasContent && !showWhenEmpty) return false;
    return true;
  }
...
function isChannelVisible(platform, channel, surface, catalog, extra) {
  ...
  if (social.channelKind(channel) === "external") return false;
...
function isCoursePublic(course, platform, catalog, forHomepage) {
  ...
  if (forHomepage && course.showOnHomepage === false) return false;
...
function resolveSectionData(section, platform, catalog) {
  ...
  if (type === "destinations") {
    const destinations = social.publicDestinations(platform, "homepage");
    return { destinations, empty: destinations.length === 0 };
  }
  ...
  if (type === "prose" || type === "program" || type === "phases" || type === "method" || type === "path" || type === "why" || type === "today") {
    return { empty: false, bodyKey: section.bodyKey || null };
  }
...
function resolveNavItem(item, platform, catalog) {
  ...
  if (source && source.kind === "channel") {
    ...
    if (social.channelKind(ch) === "external") {
      if (!social.isPublicDestination(ch)) return null;
      return {
        id: item.id,
        label: item.label || ch.label || ch.displayName,
        href: social.destinationUrl(ch),
        enabled: true,
        order: item.order,
        status: ch.status,
        source,
  ...
  if (/^https:\/\//i.test(String(href))) {
    if (social.isPlaceholderUrl(href) || !social.isHttpsUrl(href)) return null;
    return {
      id: item.id,
      label: item.label,
      href,
...
function resolveNavItems(platform, catalog, listKind) {
  ...
  for (const item of list) {
    try {
      ...
      if (next) {
        if (item.placement === "primary" || item.placement === "explore") {
          next.placement = item.placement;
        }
        resolved.push(next);
      }
    } catch {
...
function visibleCatalogItems(platform, catalog, surface) {
  const cat = catalog || emptyCatalog();
  return (cat.items || []).filter((item) => {
    ...
    if (item.topicSlug) {
      ...
      if (topic && !isTopicVisible(platform, topic, surface, cat)) return false;
    }
    const ch = findChannel(platform, item.type) || findChannel(platform, item.channelId);
    if (ch) {
      if (!isChannelVisible(platform, ch, surface, cat)) return false;
      return true;
    }
    const typeId = FORMAT_TO_CONTENT_TYPE[normalizeKey(item.type)] || item.type;
    const ct = findContentType(platform, typeId);
    if (ct && !isContentTypeVisible(platform, ct, surface, cat)) return false;
    return true;
  });
}
...
function getSearchIndexInputs(platform, catalog) {
  ...
  const items = visibleCatalogItems(platform, cat, "search");
...
function getSitemapInputs(platform, catalog) {
  ...
  const courses = publicCourses(platform, cat, true);
  const items = visibleCatalogItems(platform, cat, "sitemap");
  const corePaths = ["/"];
...
function channelRouteState(platform, key, catalog) {
  ...
  if (social.channelKind(channel) === "external") return { state: "not-found", channel };
...
function contentTypeRouteState(platform, key) {
  const contentType = findContentType(platform, key);
  if (!contentType || contentType.enabled === false) {
    return { state: "not-found", contentType: null };
  }
  const status = normalizeStatus(contentType.status);
  if (["planned", "paused", "disabled", "archived", "coming-soon"].includes(status)) {
    return { state: "not-found", contentType };
  }
  return { state: "active", contentType };
}
...
function splitPrimaryNav(items, limit) {
  ...
  const unset = [];
  for (const item of list) {
    if (item && item.placement === "explore") explicitExplore.push(item);
    else if (item && item.placement === "primary") explicitPrimary.push(item);
    else unset.push(item);
  }
  const primary = explicitPrimary.slice();
  const explore = explicitExplore.slice();
  for (const item of unset) {
    if (primary.length < cap) primary.push(item);
    else explore.push(item);
  }
  if (primary.length > cap) {
    explore.unshift.apply(explore, primary.splice(cap));
  }
  return { primary, explore, limit: cap };
}
...
module.exports = {
  ...
  publicContentTypes,
  publicChannels,
  publicDestinations: social.publicDestinations,
  publicCourses,
  comingSoonCourses,
