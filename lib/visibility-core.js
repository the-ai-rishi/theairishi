...
function isVisibleOnSurface(entity, surface, options) {
  ...
  if (status === "coming-soon") {
    if (surface === "search" || surface === "sitemap" || surface === "homepage") {
      return false;
    }
...
function getSearchIndexInputs(platform, catalog) {
  ...
  const items = (cat.items || []).filter((item) => {
    ...
    const ch = findChannel(platform, item.type);
    if (ch && !isChannelVisible(platform, ch, "search", cat)) return false;
    return true;
  });
...
function topicRouteState(platform, key, catalog) {
  ...
  if (["planned", "paused", "disabled", "archived"].includes(status)) {
    return { state: "not-found", topic };
  }
  if (status === "coming-soon") return { state: "coming-soon", topic };
  const count = topicCount(catalog, topic);
  if (status === "active" && count <= 0) return { state: "coming-soon", topic };
  return { state: "active", topic };
}

function channelRouteState(platform, key) {
  ...
  if (status === "coming-soon") return { state: "coming-soon", channel };
